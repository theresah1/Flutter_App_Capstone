...
class Conductivity extends StatefulWidget {
  const Conductivity({Key? key}) : super(key: key);

  @override
  _ConductivityState createState() => _ConductivityState();
}

class _ConductivityState extends State<Conductivity> {
  ...
  Future<void> fetchGoogleSheetData() async {
    try {
      ...
      setState(() {
        final fetchedReadings = values['conductivity'];
  ...
  Widget buildHeader(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        Text('Conductivity Data', style: GoogleFonts.poppins(fontSize: 24, fontWeight: FontWeight.bold)),
      ],
    );
  }
...
void main() {
  runApp(const MaterialApp(home: Conductivity()));
}
