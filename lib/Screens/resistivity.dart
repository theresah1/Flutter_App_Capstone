...
class Resistivity extends StatefulWidget {
  const Resistivity({super.key});

  @override
  _ResistivityState createState() => _ResistivityState();
}

class _ResistivityState extends State<Resistivity> {
  ...
  Future<void> fetchGoogleSheetData() async {
    try {
      ...
      setState(() {
        final fetchedReadings = values['resistivity'];
  ...
  Widget buildHeader(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        Text('Resistivity Data', style: GoogleFonts.poppins(fontSize: 24, fontWeight: FontWeight.bold)),
      ],
    );
  }
...
void main() {
  runApp(const MaterialApp(home: Resistivity()));
}
